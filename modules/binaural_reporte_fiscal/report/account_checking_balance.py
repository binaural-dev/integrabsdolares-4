...
class ReportAccountCheckingBalance(models.AbstractModel):
    ...
    def _get_templates(self):
        templates = super(ReportAccountCheckingBalance, self)._get_templates()
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining, balance_progress):
        ...
        amls_query, amls_params = self._get_query_amls(options, expanded_account, offset=offset,
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(ReportAccountCheckingBalance, self)._get_options_domain(options)
    ...
    def _get_options_sum_balance(self, options):
        ...
        fiscalyear_dates = self.env.company.compute_fiscalyear_dates(
            fields.Date.from_string(new_options['date']['date_from']))
        new_options['date'] = {
            'mode': 'range',
